...
```{r, engine='bash'}
echo "${GENOME_GTF}" "${GTF_SOURCE}"
```

```{r gtf, engine='bash'}


curl -L -o "${GENOME_GTF}" "${GTF_SOURCE}"
```

```{r gff, engine='bash'}


curl -L -o "${GENOME_GFF}" "${GFF_SOURCE}"
```
...
awk '{
  count++                                # total entries
  key=$1 FS $4 FS $5                     # unique gene key
  seen[key]++                            # count unique genes
  len=$5-$4+1
  if (min=="" || len<min) min=len        # track min
  if (len>max) max=len                   # track max
  sum+=len                               # accumulate for average
}
END {
  print "Total entries:", count
  print "Unique genes (by col1,col4,col5):", length(seen)
...
```{bash}
awk '$4==0 || $5==0 || $4==$5' "${OUTPUT_DIR}/lncRNA.gtf" | head
```

```{bash}
cut -f1,4,5 "${OUTPUT_DIR}/lncRNA.gtf" | sort | uniq -d | head
```

Drop duplicates

```{bash}
awk '!seen[$1,$4,$5]++' "${OUTPUT_DIR}/lncRNA.gtf" > "${OUTPUT_DIR}/lncRNA.gtf.tmp" \
  && mv "${OUTPUT_DIR}/lncRNA.gtf.tmp" "${OUTPUT_DIR}/lncRNA.gtf"
```
...
removes lines with less the 10x coverage on half of samples
```{bash}
